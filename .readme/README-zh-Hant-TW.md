...
建置過程從 ModelScope 的 RapidOCR v3.9.2 下載並驗證以下 4 個資產:
...
* `提示` PP-OCRv6 Small 偵測模型, 辨識模型, 輔助分類模型和字典在兩個版本中的 SHA-256 均一致; 本次更新模型來源紀錄, 辨識行為保持不變
* `相依性` RapidOCR 模型來源版本從 v3.9.1 升級至 v3.9.2, 並在 `version.properties` 中統一鎖定模型版本
